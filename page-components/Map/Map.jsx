...
function basicRender(
  point,
  percent = 50,
  imageUrl = 'https://hft-staging.s3.ap-southeast-1.amazonaws.com/IMAGES/POST/1910543b1091e5c1fb-123457qweryu_thumbnail_small.png'
) {
  const el = document.createElement('div');
  // el.className = `custom_marker ${style.custom_marker}`;
  el.innerHTML = `
  <div class="${style.custom_marker}">
    <div class="${style.circle_container}">
      <svg class="${style.circle}" width="30" height="30" viewBox="0 0 30 30" style="--image-url: url('${imageUrl}');">
        ...
        <circle cx="15" cy="15" r="13" fill="transparent" stroke="#FF981F" stroke-width="4" style="--percent: ${percent};" />
      </svg>
    </div>
    <div class="${style.rectangle}">
      <div class="${style.text}">${point.content}</div>
    ...
    const updateMarkers = (newData) => {
      ...
      points.forEach((point) => {
        ...
        marker.getElement().addEventListener('click', () => {
          handleMarkerClick(point.lng, point.lat);
        });
